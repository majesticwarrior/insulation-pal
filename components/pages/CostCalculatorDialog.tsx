...
// Simple state → climate zone mapping (approximate). For brevity, one zone per state.
...
function suggestRValue(zone: number, area: Area): number {
  // Approximations inspired by DOE recommendations
...
// Simple pricing in $/sqft for materials
...
// Labor $/sqft (installed cost adds this to materials)
...
// Adjust labor rate for Fiberglass Batts
...
export default function CostCalculatorDialog({ children }: Props) {
  ...
  // Keep r-value suggestion in sync when area/state change, but don't override manual edits until type/area/state changes
  useMemo(() => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>Average Cost Calculator</DialogTitle>
        </DialogHeader>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-3">
            <div>
              <Label htmlFor="sqft">House Size (sq ft)</Label>
              <Input id="sqft" inputMode="numeric" value={sqft} onChange={(e) => setSqft(e.target.value === '' ? '' : Number(e.target.value))} placeholder="e.g., 2000" />
            </div>
            <div>
            ...
            <div>
              ...
              <Input id="rvalue" inputMode="numeric" value={rValue} onChange={(e) => setRValue(Number(e.target.value) || 0)} />
          ...
          <div className="space-y-3">
            <Card>
              <CardContent className="p-4">
                <div className="flex justify-between text-sm mb-2"><span>Average Material Rate</span><span>${results.materialRate.toFixed(2)}/sq ft</span></div>
                <div className="flex justify-between text-sm mb-2"><span>Average Labor Rate</span><span>${results.laborRate.toFixed(2)}/sq ft</span></div>
                <div className="flex justify-between text-sm"><span>Square Footage</span><span>{typeof sqft === 'number' ? sqft : 0} sq ft</span></div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="flex justify-between font-semibold text-[#0a4768] mb-2"><span>DIY Price (Materials)</span><span>${results.diy.toFixed(2)}</span></div>
                <div className="flex justify-between font-semibold text-[#0a4768]"><span>Installed Price (With Labor)</span><span>${results.installed.toFixed(2)}</span></div>
              </CardContent>
            </Card>
