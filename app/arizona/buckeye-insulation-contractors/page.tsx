...
// Generate dynamic metadata based on Buckeye contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getBuckeyeContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Buckeye Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Buckeye, AZ'
    
    return {
      title: `Buckeye Insulation Contractors, Find Top Rated Local Companies Near You`,
      ...
      keywords: [
        ...
        'Buckeye attic insulation',
        'Buckeye spray foam insulation',
        'Buckeye wall insulation',
        'Buckeye basement insulation',
        'Buckeye crawl space insulation',
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/buckeye-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Buckeye Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Buckeye, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Buckeye contractors from Supabase database
async function getBuckeyeContractors() {
  try {
    // Get contractors based in Buckeye (business city from their profile)
    ...
    // Filter contractors who serve Buckeye:
    // 1. Contractors based in Buckeye (business_city matches)
    // 2. Contractors who have Buckeye in their service areas
    const targetCity = 'buckeye'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const buckeyeContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Buckeye
      const isBasedInBuckeye = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Buckeye via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesBuckeye = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInBuckeye || servesBuckeye
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = buckeyeContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Buckeye
        const isBasedInBuckeye = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInBuckeye // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch recent insulation projects completed in Buckeye
async function getBuckeyeRecentProjects() {
  try {
...
// Fetch Buckeye contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Buckeye
    ...
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Buckeye%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Buckeye reviews found, fallback to Phoenix reviews
...
// Add Buckeye area cities with 50k+ population
const buckeyeAreaCities = [
...
export default async function BuckeyeInsulationContractors() {
  const schemas = createCitySchemas('Buckeye', 'Arizona', '/arizona/buckeye-insulation-contractors')
  const mapUrl = getCityMapUrl('buckeye', 'Buckeye, AZ')

  const buckeyeContractors = await getBuckeyeContractors()
  const recentProjects = await getBuckeyeRecentProjects()
  const phoenixReviews = await getPhoenixReviews()
  
  const cityStats = {
    totalReviews: buckeyeContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: buckeyeContractors.length > 0 ? 
      (buckeyeContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / buckeyeContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: buckeyeContractors.length,
    jobsCompleted: buckeyeContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-700 mb-8">
              ...
              licensed professionals who understand Buckeye's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Buckeye</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4">
          ...
          <div className="space-y-8">
            {buckeyeContractors.map((contractor: any) => (
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {buckeyeAreaCities.map((city, index) => (
              city.name === 'Buckeye' ? (
        ...
        <div className="container mx-auto px-4">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Buckeye, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Buckeye and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Buckeye,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600">
              See what customers are saying about our Buckeye insulation contractors
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Buckeye Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Buckeye's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Buckeye Quotes Now
          </QuoteButton>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Learn the basics of home insulation and why it's crucial for Buckeye homes
            </p>
          </div>
