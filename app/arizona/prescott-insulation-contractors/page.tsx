...
// Generate dynamic metadata based on Prescott contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getPrescottContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Prescott Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Prescott, AZ'
    
    return {
      title: 'Insulation Contractors in Prescott, AZ, Expert Licensed Companies - InsulationPal',
      ...
      keywords: [
        ...
        'Prescott attic insulation',
        'Prescott spray foam insulation',
        'Prescott wall insulation',
        'Prescott basement insulation',
        'Prescott crawl space insulation',
        'Prescott energy efficiency',
        'Arizona insulation services',
        'Prescott insulation quotes',
        'licensed insulation contractors Prescott',
        'verified insulation professionals Prescott'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/prescott-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Prescott Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Prescott, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Prescott contractors from Supabase database
async function getPrescottContractors() {
  try {
    // Get contractors based in Prescott (business city from their profile)
    ...
    // Filter contractors who serve Prescott:
    // 1. Contractors based in Prescott (business_city matches)
    // 2. Contractors who have Prescott in their service areas
    const targetCity = 'prescott'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const prescottContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Prescott
      const isBasedInPrescott = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Prescott via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesPrescott = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInPrescott || servesPrescott
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = prescottContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Prescott
        const isBasedInPrescott = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInPrescott // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch recent insulation projects completed in Prescott
async function getPrescottRecentProjects() {
  try {
...
// Fetch Prescott contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Prescott
    ...
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Prescott%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Prescott reviews found, fallback to Phoenix reviews
...
// Add Prescott area cities with 50k+ population
const prescottAreaCities = [
  { name: 'Prescott', population: '45,827', slug: 'prescott' },
  { name: 'Flagstaff', population: '76,989', slug: 'flagstaff' },
  { name: 'Phoenix', population: '1,608,139', slug: 'phoenix' },
  { name: 'Sedona', population: '9,684', slug: 'sedona' }
]
...
export default async function PrescottInsulationContractors() {
  ...
  const serviceDescriptions = getCityServiceDescriptions('Prescott')

  const prescottContractors = await getPrescottContractors()
  const recentProjects = await getPrescottRecentProjects()
  const phoenixReviews = await getPhoenixReviews()
  
  const cityStats = {
    totalReviews: prescottContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: prescottContractors.length > 0 ? 
      (prescottContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / prescottContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: prescottContractors.length,
    jobsCompleted: prescottContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-bold text-[#0a4768] mb-6">
              Hire a Licensed Insulation Contractor in Prescott
            </h1>
            <p className="text-xl text-gray-700 mb-8">
              Find the best insulation contractors in Prescott, Arizona. Get free quotes from local, 
              licensed professionals who understand Prescott's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Prescott</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4 max-w-[1400px]">
          ...
          <div className="space-y-8">
            {prescottContractors.map((contractor: any) => (
      ...
      {/* Finding Reliable Insulation Contractors in Prescott */}
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="space-y-6">
            <p className="text-gray-700 leading-relaxed">
              Finding reliable insulation contractors in Prescott isn't always a walk in the park. Homeowners have to double-check licenses, weigh prices, and try to figure out which company actually delivers good work. InsulationPal now teams up with local insulation contractors in Prescott, linking residents with licensed, verified pros through a platform that brings up to three competitive bids for any insulation project. It takes a lot of the guesswork out of hiring insulation services and, honestly, gives local businesses a boost too.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              The platform's built for Prescott homeowners needing attic insulation, crawl space upgrades, spray foam, or even big commercial jobs. We match folks with pre-screened insulation contractors in Arizona who've got the right licenses and insurance—so you're not left wondering if you missed something important.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              Working with established local businesses in Prescott, we open the door to quality insulation services without all the endless Googling or stress over paperwork. The process is pretty simple: compare qualified pros, see what fits, and decide who gets the job. No need to overthink it.
            </p>
            
            <Card className="bg-[#D8E1FF] border-[#0a4768] shadow-lg mt-8">
              <CardContent className="p-6 text-center">
                <CheckCircle className="w-12 h-12 text-[#0a4768] mx-auto mb-4" />
                <h3 className="text-2xl font-bold text-[#0a4768] mb-3">Ready to Find Your Prescott Contractor?</h3>
                <p className="text-lg text-gray-700 mb-2">
                  Connect with pre-screened, licensed, and insured insulation professionals in Prescott, AZ.
                </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {prescottAreaCities.map((city, index) => (
              city.name === 'Prescott' ? (
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Prescott, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Prescott and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Prescott,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Prescott Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Prescott insulation contractors
            </p>
          </div>
            ...
            <div className="max-w-[1400px] mx-auto">
              ...
              <div className="text-center mt-8">
                <p className="text-gray-600 text-sm">
                  Showing {phoenixReviews.length} of the latest reviews from Prescott customers
                </p>
              </div>
            </div>
          ) : (
      ...
      {/* Recent Prescott Completed Projects */}
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              See the quality work performed by our verified contractors in the Prescott area
            </p>
          </div>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Comprehensive insulation solutions for every part of your Prescott home
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Prescott Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Prescott's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Prescott Quotes Now
          </QuoteButton>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Learn the basics of home insulation and why it's crucial for Prescott homes
            </p>
          </div>
