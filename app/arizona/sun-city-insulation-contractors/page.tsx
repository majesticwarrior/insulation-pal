...
// Generate dynamic metadata based on Sun City contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getSunCityContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Sun City Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Sun City, AZ'
    
    return {
      title: `Sun City Insulation Contractors, Find Top Rated Local Companies Near You`,
      ...
      keywords: [
        ...
        'Sun City attic insulation',
        'Sun City spray foam insulation',
        'Sun City wall insulation',
        'Sun City basement insulation',
        'Sun City crawl space insulation',
        'Sun City energy efficiency',
        'Arizona insulation services',
        'Sun City insulation quotes',
        'licensed insulation contractors Sun City',
        'verified insulation professionals Sun City'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/sun-city-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Sun City Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Sun City, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Sun City contractors from Supabase database
async function getSunCityContractors() {
  try {
    // Get contractors based in Sun City (business city from their profile)
    ...
    // Filter contractors who serve Sun City:
    // 1. Contractors based in Sun City (business_city matches)
    // 2. Contractors who have Sun City in their service areas
    const targetCity = 'sun city'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const sunCityContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Sun City
      const isBasedInSunCity = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Sun City via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesSunCity = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInSunCity || servesSunCity
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = sunCityContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Sun City
        const isBasedInSunCity = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInSunCity // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch recent insulation projects completed in Sun City
async function getSunCityRecentProjects() {
  try {
...
// Fetch Sun City contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Sun City
    ...
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Sun City%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Sun City reviews found, fallback to Phoenix reviews
...
// Add Sun City area cities with 50k+ population
const sunCityAreaCities = [
  { name: 'Sun City', population: '38,309', slug: 'sun-city' },
...
export default async function SunCityInsulationContractors() {
  const schemas = createCitySchemas('Sun City', 'Arizona', '/arizona/sun-city-insulation-contractors')
  const mapUrl = getCityMapUrl('sun-city', 'Sun City, AZ')
  
  const sunCityContractors = await getSunCityContractors()
  const recentProjects = await getSunCityRecentProjects()
  const phoenixReviews = await getPhoenixReviews()
  
  const cityStats = {
    totalReviews: sunCityContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: sunCityContractors.length > 0 ? 
      (sunCityContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / sunCityContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: sunCityContractors.length,
    jobsCompleted: sunCityContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-700 mb-8">
              ...
              licensed professionals who understand Sun City's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Sun City</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <h2 className="text-3xl font-bold text-[#0a4768] mb-12 text-center">
            Top Sun City Insulation Contractors
          </h2>
          <div className="space-y-8">
            {sunCityContractors.map((contractor: any) => (
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {sunCityAreaCities.map((city, index) => (
              city.name === 'Sun City' ? (
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Sun City, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Sun City and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Sun City Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Sun City insulation contractors
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Sun City Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Sun City's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Sun City Quotes Now
          </QuoteButton>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Learn the basics of home insulation and why it's crucial for Sun City homes
            </p>
          </div>
