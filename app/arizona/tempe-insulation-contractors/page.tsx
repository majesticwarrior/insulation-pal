...
// Generate dynamic metadata based on Tempe contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getTempeContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Tempe Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Tempe, AZ'
    
    return {
      title: `Tempe Insulation Contractors, Find Top Rated Local Companies Near You`,
      ...
      keywords: [
        ...
        'Tempe attic insulation',
        'Tempe spray foam insulation',
        'Tempe wall insulation',
        'Tempe basement insulation',
        'Tempe crawl space insulation',
        'Tempe energy efficiency',
        'Arizona insulation services',
        'Tempe insulation quotes',
        'licensed insulation contractors Tempe',
        'verified insulation professionals Tempe'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/tempe-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Tempe Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Tempe, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Tempe contractors from Supabase database
async function getTempeContractors() {
  try {
    // Get contractors based in Tempe (business city from their profile)
    ...
    // Filter contractors who serve Tempe:
    // 1. Contractors based in Tempe (business_city matches)
    // 2. Contractors who have Tempe in their service areas
    const targetCity = 'tempe'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const tempeContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Tempe
      const isBasedInTempe = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Tempe via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesTempe = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInTempe || servesTempe
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = tempeContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Tempe
        const isBasedInTempe = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInTempe // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch Tempe contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Tempe
    ...
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Tempe%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Tempe reviews found, fallback to Phoenix reviews
...
// Add Tempe area cities with 50k+ population
const tempeAreaCities = [
...
export default async function TempeInsulationContractors() {
  const schemas = createCitySchemas('Tempe', 'Arizona', '/arizona/tempe-insulation-contractors')
  ...
  const cityStats = {
    totalReviews: tempeContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: tempeContractors.length > 0 ? 
      (tempeContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / tempeContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: tempeContractors.length,
    jobsCompleted: tempeContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-700 mb-8">
              ...
              licensed professionals who understand Tempe's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Tempe</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-[#0a4768] mb-12 text-center">
            Top Tempe Insulation Contractors
          </h2>
          <div className="space-y-8">
            {tempeContractors.map((contractor: any) => (
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {tempeAreaCities.map((city, index) => (
        ...
        <div className="container mx-auto px-4">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Tempe, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Tempe and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Tempe,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Tempe Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Tempe insulation contractors
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Tempe Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Tempe's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Tempe Quotes Now
          </QuoteButton>
        </div>
      </section>
