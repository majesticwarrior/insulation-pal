...
// Generate dynamic metadata based on Gilbert contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getGilbertContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Gilbert Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Gilbert, AZ'
    
    return {
      title: `Gilbert Insulation Contractors, Find Top Rated Local Companies Near You`,
      ...
      keywords: [
        ...
        'Gilbert attic insulation',
        'Gilbert spray foam insulation',
        'Gilbert wall insulation',
        'Gilbert basement insulation',
        'Gilbert crawl space insulation',
        'Gilbert energy efficiency',
        'Arizona insulation services',
        'Gilbert insulation quotes',
        'licensed insulation contractors Gilbert',
        'verified insulation professionals Gilbert'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/gilbert-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Gilbert Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Gilbert, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Gilbert contractors from Supabase database
async function getGilbertContractors() {
  try {
    // Get contractors based in Gilbert (business city from their profile)
    ...
    // Filter contractors who serve Gilbert:
    // 1. Contractors based in Gilbert (business_city matches)
    // 2. Contractors who have Gilbert in their service areas
    const targetCity = 'gilbert'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const gilbertContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Gilbert
      const isBasedInGilbert = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Gilbert via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesGilbert = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInGilbert || servesGilbert
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = gilbertContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Gilbert
        const isBasedInGilbert = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInGilbert // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch Gilbert contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Gilbert
    const { data: locationReviews, error: locationError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('location', '%Gilbert%')
      .order('created_at', { ascending: false })
      .limit(10)

    // Try to fetch reviews from contractors based in Gilbert
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Gilbert%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Gilbert reviews found, fallback to Phoenix reviews
...
// Add Gilbert area cities with 50k+ population
const gilbertAreaCities = [
...
export default async function GilbertInsulationContractors() {
  const schemas = createCitySchemas('Gilbert', 'Arizona', '/arizona/gilbert-insulation-contractors')
  const mapUrl = getCityMapUrl('gilbert', 'Gilbert, AZ')
  ...
  const cityStats = {
    totalReviews: gilbertContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: gilbertContractors.length > 0 ? 
      (gilbertContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / gilbertContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: gilbertContractors.length,
    jobsCompleted: gilbertContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-700 mb-8">
              ...
              licensed professionals who understand Gilbert's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Gilbert</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-[#0a4768] mb-12 text-center">
            Top Gilbert Insulation Contractors
          </h2>
          <div className="space-y-8">
            {gilbertContractors.map((contractor: any) => (
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {gilbertAreaCities.map((city, index) => (
        ...
        <div className="container mx-auto px-4">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Gilbert, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Gilbert and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Gilbert,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Gilbert Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Gilbert insulation contractors
            </p>
          </div>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Comprehensive insulation solutions for every part of your Gilbert home
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Gilbert Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Gilbert's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Gilbert Quotes Now
          </QuoteButton>
        </div>
      </section>
