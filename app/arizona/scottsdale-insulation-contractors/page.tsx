...
// Generate dynamic metadata based on Scottsdale contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getScottsdaleContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Scottsdale Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Scottsdale, AZ'
    
    return {
      title: `Scottsdale Insulation Contractors, Find Top Rated Local Companies Near You`,
      ...
      keywords: [
        ...
        'Scottsdale attic insulation',
        'Scottsdale spray foam insulation',
        'Scottsdale wall insulation',
        'Scottsdale basement insulation',
        'Scottsdale crawl space insulation',
        'Scottsdale energy efficiency',
        'Arizona insulation services',
        'Scottsdale insulation quotes',
        'licensed insulation contractors Scottsdale',
        'verified insulation professionals Scottsdale'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/scottsdale-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Scottsdale Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Scottsdale, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Scottsdale contractors from Supabase database
async function getScottsdaleContractors() {
  try {
    // Get contractors based in Scottsdale (business city from their profile)
    ...
    // Filter contractors who serve Scottsdale:
    // 1. Contractors based in Scottsdale (business_city matches)
    // 2. Contractors who have Scottsdale in their service areas
    const targetCity = 'scottsdale'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const scottsdaleContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Scottsdale
      const isBasedInScottsdale = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Scottsdale via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesScottsdale = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInScottsdale || servesScottsdale
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = scottsdaleContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Scottsdale
        const isBasedInScottsdale = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInScottsdale // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch recent insulation projects completed in Scottsdale
async function getScottsdaleRecentProjects() {
  try {
...
// Fetch Scottsdale contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Scottsdale
    const { data: locationReviews, error: locationError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('location', '%Scottsdale%')
      .order('created_at', { ascending: false })
      .limit(10)

    // Try to fetch reviews from contractors based in Scottsdale
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Scottsdale%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Scottsdale reviews found, fallback to Phoenix reviews
...
// Add Scottsdale area cities with 50k+ population
const scottsdaleAreaCities = [
...
export default async function ScottsdaleInsulationContractors() {
  ...
  const serviceDescriptions = getCityServiceDescriptions('Scottsdale')

  const scottsdaleContractors = await getScottsdaleContractors()
  const recentProjects = await getScottsdaleRecentProjects()
  const phoenixReviews = await getPhoenixReviews()
  
  const cityStats = {
    totalReviews: scottsdaleContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: scottsdaleContractors.length > 0 ? 
      (scottsdaleContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / scottsdaleContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: scottsdaleContractors.length,
    jobsCompleted: scottsdaleContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-700 mb-8">
              ...
              licensed professionals who understand Scottsdale's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Scottsdale</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4">
          ...
          <div className="space-y-8">
            {scottsdaleContractors.map((contractor: any) => (
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {scottsdaleAreaCities.map((city, index) => (
        ...
        <div className="container mx-auto px-4">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Scottsdale, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Scottsdale and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Scottsdale,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Scottsdale Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Scottsdale insulation contractors
            </p>
          </div>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Comprehensive insulation solutions for every part of your Scottsdale home
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Scottsdale Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Scottsdale's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Scottsdale Quotes Now
          </QuoteButton>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Learn the basics of home insulation and why it's crucial for Scottsdale homes
            </p>
          </div>
