...
// Generate dynamic metadata based on Tucson contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getTucsonContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Tucson Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Tucson, AZ'
    
    return {
      title: `Tucson Insulation Contractors, Find Top Rated Local Companies Near You`,
      ...
      keywords: [
        ...
        'Tucson attic insulation',
        'Tucson spray foam insulation',
        'Tucson wall insulation',
        'Tucson basement insulation',
        'Tucson crawl space insulation',
        'Tucson energy efficiency',
        'Arizona insulation services',
        'Tucson insulation quotes',
        'licensed insulation contractors Tucson',
        'verified insulation professionals Tucson'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/tucson-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Tucson Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Tucson, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Tucson contractors from Supabase database
async function getTucsonContractors() {
  try {
    // Get contractors based in Tucson (business city from their profile)
    ...
    if (error) {
      ...
      if (error.message && error.message.includes('bbb_accredited')) {
        ...
        // Filter contractors who serve Tucson:
        // 1. Contractors based in Tucson (business_city matches)
        // 2. Contractors who have Tucson in their service areas
        const targetCity = 'tucson'
        const targetCityNormalized = targetCity.toLowerCase().trim()
        
        const tucsonContractorsRetry = contractorsRetry?.filter((contractor: any) => {
          // Check if contractor is based in Tucson
          const isBasedInTucson = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
          
          // Check if contractor serves Tucson via service areas
          const serviceAreas = contractor.contractor_service_areas || []
          const servesTucson = serviceAreas.some((area: any) => {
            const areaCity = area.city?.toLowerCase().trim()
            return areaCity === targetCityNormalized
          })
          
          return isBasedInTucson || servesTucson
        }) || []
        ...
        const uniqueContractors = tucsonContractorsRetry.reduce((acc: any[], contractor: any) => {
          ...
          if (!existingContractor) {
            ...
            acc.push({
              ...
              isBasedInCity: isBasedInTucson // Flag to sort city-based contractors first
    ...
    const tucsonContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Tucson
      const isBasedInTucson = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
    ...
    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = tucsonContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Tucson
        const isBasedInTucson = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
...
// Fetch Tucson contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Tucson
    ...
    // Try to fetch reviews from contractors based in Tucson
    const { data: contractorReviews, error: contractorError} = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Tucson%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Tucson reviews found, fallback to Phoenix reviews
...
// Add Tucson area cities with 50k+ population
const tucsonAreaCities = [
...
export default async function TucsonInsulationContractors() {
  const schemas = createCitySchemas('Tucson', 'Arizona', '/arizona/tucson-insulation-contractors')
  const mapUrl = getCityMapUrl('tucson', 'Tucson, AZ')
  ...
  const cityStats = {
    totalReviews: tucsonContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: tucsonContractors.length > 0 ? 
      (tucsonContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / tucsonContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: tucsonContractors.length,
    jobsCompleted: tucsonContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-700 mb-8">
              ...
              licensed professionals who understand Tucson's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Tucson</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-[#0a4768] mb-12 text-center">
            Top Tucson Insulation Contractors
          </h2>
          <div className="space-y-8">
            {tucsonContractors.map((contractor: any) => (
        ...
        <div className="container mx-auto px-4">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Tucson, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Tucson and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Tucson,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Tucson Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Tucson insulation contractors
            </p>
          </div>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Comprehensive insulation solutions for every part of your Tucson home
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Tucson Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Tucson's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Tucson Quotes Now
          </QuoteButton>
        </div>
      </section>
