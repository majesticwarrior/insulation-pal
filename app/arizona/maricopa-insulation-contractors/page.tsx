...
async function getMaricopaContractors() {
  try {
    ...
    // Filter contractors who serve Maricopa:
    // 1. Contractors based in Maricopa (business_city matches)
    // 2. Contractors who have Maricopa in their service areas
    const targetCity = 'maricopa'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const maricopaContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Maricopa
      const isBasedInMaricopa = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Maricopa via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesMaricopa = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInMaricopa || servesMaricopa
    }) || []
    ...
    const uniqueContractors = maricopaContractors.reduce((acc: any[], contractor: any) => {
      ...
      if (!existingContractor) {
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInMaricopa // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
export default async function MaricopaInsulationContractors() {
  const schemas = createCitySchemas('Maricopa', 'Arizona', '/arizona/maricopa-insulation-contractors')
