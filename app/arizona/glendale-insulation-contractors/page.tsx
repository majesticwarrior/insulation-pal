...
// Generate dynamic metadata based on Glendale contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getGlendaleContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Glendale Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Glendale, AZ'
    
    return {
      title: 'Insulation Contractors in Glendale, AZ, Expert Licensed Companies - InsulationPal',
      ...
      keywords: [
        ...
        'Glendale attic insulation',
        'Glendale spray foam insulation',
        'Glendale wall insulation',
        'Glendale basement insulation',
        'Glendale crawl space insulation',
        'Glendale energy efficiency',
        'Arizona insulation services',
        'Glendale insulation quotes',
        'licensed insulation contractors Glendale',
        'verified insulation professionals Glendale'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/glendale-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Glendale Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Glendale, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Glendale contractors from Supabase database
async function getGlendaleContractors() {
  try {
    // Get contractors based in Glendale (business city from their profile)
    ...
    // Filter contractors who serve Glendale:
    // 1. Contractors based in Glendale (business_city matches)
    // 2. Contractors who have Glendale in their service areas
    const targetCity = 'glendale'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const glendaleContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Glendale
      const isBasedInGlendale = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Glendale via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesGlendale = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInGlendale || servesGlendale
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = glendaleContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Glendale
        const isBasedInGlendale = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInGlendale // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch recent insulation projects completed in Glendale
async function getGlendaleRecentProjects() {
  try {
...
// Fetch Glendale contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Glendale
    const { data: locationReviews, error: locationError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('location', '%Glendale%')
      .order('created_at', { ascending: false })
      .limit(10)

    // Try to fetch reviews from contractors based in Glendale
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Glendale%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Glendale reviews found, fallback to Phoenix reviews
...
// Add Glendale area cities with 50k+ population
const glendaleAreaCities = [
...
export default async function GlendaleInsulationContractors() {
  const schemas = createCitySchemas('Glendale', 'Arizona', '/arizona/glendale-insulation-contractors')
  const mapUrl = getCityMapUrl('glendale', 'Glendale, AZ')
  const serviceDescriptions = getCityServiceDescriptions('Glendale')

  const glendaleContractors = await getGlendaleContractors()
  const recentProjects = await getGlendaleRecentProjects()
  const phoenixReviews = await getPhoenixReviews()
  
  const cityStats = {
    totalReviews: glendaleContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: glendaleContractors.length > 0 ? 
      (glendaleContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / glendaleContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: glendaleContractors.length,
    jobsCompleted: glendaleContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-bold text-[#0a4768] mb-6">
              Hire a Licensed Insulation Contractor in Glendale
            </h1>
            <p className="text-xl text-gray-700 mb-8">
              Find the best insulation contractors in Glendale, Arizona. Get free quotes from local, 
              licensed professionals who understand Glendale's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Glendale</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <h2 className="text-3xl font-bold text-[#0a4768] mb-12 text-center">
            Top Glendale Insulation Contractors
          </h2>
          <div className="space-y-8">
            {glendaleContractors.map((contractor: any) => (
      ...
      {/* Finding Reliable Insulation Contractors in Glendale */}
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="space-y-6">
            <p className="text-gray-700 leading-relaxed">
              Finding reliable insulation contractors in Glendale can be a real headache for homeowners and businesses hoping to boost energy efficiency and comfort. It often means sifting through endless search results, making calls, and still wondering if you're actually getting a good deal or trustworthy service.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              InsulationPal now partners with local insulation contractors throughout Glendale to make it easier for residents to connect with licensed professionals for their insulation projects. The platform brings together skilled contractors who handle everything from attic insulation to full building retrofits, whether it's for a home or a business.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              We've set up this partnership network to help folks in Glendale skip the usual hassles that come with starting an insulation project. Through our platform, you can browse bids from vetted local insulation pros, compare what they offer, and decide what works best for your needs—without the usual guesswork.
            </p>
            
            <Card className="bg-[#D8E1FF] border-[#0a4768] shadow-lg mt-8">
              <CardContent className="p-6 text-center">
                <CheckCircle className="w-12 h-12 text-[#0a4768] mx-auto mb-4" />
                <h3 className="text-2xl font-bold text-[#0a4768] mb-3">Ready to Find Your Glendale Contractor?</h3>
                <p className="text-lg text-gray-700 mb-2">
                  Connect with pre-screened, licensed, and insured insulation professionals in Glendale, AZ.
                </p>
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Glendale, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Glendale and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Glendale,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Glendale Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Glendale insulation contractors
            </p>
          </div>
            ...
            <div className="max-w-[1400px] mx-auto">
              ...
              <div className="text-center mt-8">
                <p className="text-gray-600 text-sm">
                  Showing {phoenixReviews.length} of the latest reviews from Glendale customers
                </p>
              </div>
            </div>
          ) : (
      ...
      {/* Recent Glendale Completed Projects */}
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              See the quality work performed by our verified contractors in the Glendale area
            </p>
          </div>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Comprehensive insulation solutions for every part of your Glendale home
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Glendale Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Glendale's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Glendale Quotes Now
          </QuoteButton>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Learn the basics of home insulation and why it's crucial for Glendale homes
            </p>
          </div>
