...
// Generate dynamic metadata based on Goodyear contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getGoodyearContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Goodyear Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Goodyear, AZ'
    
    return {
      title: 'Insulation Contractors in Goodyear, AZ, Expert Licensed Companies - InsulationPal',
      ...
      keywords: [
        'Goodyear insulation contractors',
        'Goodyear attic insulation',
        'Goodyear spray foam insulation',
        'Goodyear wall insulation',
        'Goodyear basement insulation',
        'Goodyear crawl space insulation',
        'Goodyear energy efficiency',
        'Arizona insulation services',
        'Goodyear insulation quotes',
        'licensed insulation contractors Goodyear',
        'verified insulation professionals Goodyear'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/goodyear-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Goodyear Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Goodyear, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Goodyear contractors from Supabase database
async function getGoodyearContractors() {
  try {
    // Get contractors based in Goodyear (business city from their profile)
    ...
    // Filter contractors who serve Goodyear:
    // 1. Contractors based in Goodyear (business_city matches)
    // 2. Contractors who have Goodyear in their service areas
    const targetCity = 'goodyear'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const goodyearContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Goodyear
      const isBasedInGoodyear = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Goodyear via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesGoodyear = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInGoodyear || servesGoodyear
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = goodyearContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Goodyear
        const isBasedInGoodyear = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInGoodyear // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch recent insulation projects completed in Goodyear
async function getGoodyearRecentProjects() {
  try {
...
// Fetch Goodyear contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Goodyear
    ...
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Goodyear%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Goodyear reviews found, fallback to Phoenix reviews
...
// Add Goodyear area cities with 50k+ population
const goodyearAreaCities = [
...
export default async function GoodyearInsulationContractors() {
  const schemas = createCitySchemas('Goodyear', 'Arizona', '/arizona/goodyear-insulation-contractors')
  const mapUrl = getCityMapUrl('goodyear', 'Goodyear, AZ')

  const goodyearContractors = await getGoodyearContractors()
  const recentProjects = await getGoodyearRecentProjects()
  const phoenixReviews = await getPhoenixReviews()
  
  const cityStats = {
    totalReviews: goodyearContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: goodyearContractors.length > 0 ? 
      (goodyearContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / goodyearContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: goodyearContractors.length,
    jobsCompleted: goodyearContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-bold text-[#0a4768] mb-6">
              Hire a Licensed Insulation Contractor in Goodyear
            </h1>
            <p className="text-xl text-gray-700 mb-8">
              Find the best insulation contractors in Goodyear, Arizona. Get free quotes from local, 
              licensed professionals who understand Goodyear's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Goodyear</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <h2 className="text-3xl font-bold text-[#0a4768] mb-12 text-center">
            Top Goodyear Insulation Contractors
          </h2>
          <div className="space-y-8">
            {goodyearContractors.map((contractor: any) => (
      ...
      {/* Finding Reliable Insulation Contractors in Goodyear */}
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="space-y-6">
            <p className="text-gray-700 leading-relaxed">
              Finding reliable insulation contractors in Goodyear isn't always straightforward, especially for homeowners who want good work without breaking the bank. Local businesses often just don't get the visibility they need, and folks at home are left wondering who to trust for their insulation projects.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              InsulationPal now connects Goodyear homeowners with vetted local insulation contractors through our free digital platform, making it easier to find qualified professionals for any insulation project. We're that missing link between residents and contractors who actually get what Goodyear's climate demands. Our platform takes out the guesswork, giving you access to licensed pros who know their stuff.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              We set this up to help both homeowners and local insulation contractors. Residents can check out a few quotes and support local businesses at the same time. Honestly, it just feels better when you know your project supports the community and you're getting solid work at a fair price.
            </p>
            
            <Card className="bg-[#D8E1FF] border-[#0a4768] shadow-lg mt-8">
              <CardContent className="p-6 text-center">
                <CheckCircle className="w-12 h-12 text-[#0a4768] mx-auto mb-4" />
                <h3 className="text-2xl font-bold text-[#0a4768] mb-3">Ready to Find Your Goodyear Contractor?</h3>
                <p className="text-lg text-gray-700 mb-2">
                  Connect with pre-screened, licensed, and insured insulation professionals in Goodyear, AZ.
                </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {goodyearAreaCities.map((city, index) => (
              city.name === 'Goodyear' ? (
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Goodyear, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Goodyear and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Goodyear,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Goodyear Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Goodyear insulation contractors
            </p>
          </div>
            ...
            <div className="max-w-[1400px] mx-auto">
              ...
              <div className="text-center mt-8">
                <p className="text-gray-600 text-sm">
                  Showing {phoenixReviews.length} of the latest reviews from Goodyear customers
                </p>
              </div>
            </div>
          ) : (
      ...
      {/* Recent Goodyear Completed Projects */}
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Goodyear Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Goodyear's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Goodyear Quotes Now
          </QuoteButton>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Learn the basics of home insulation and why it's crucial for Goodyear homes
            </p>
          </div>
