...
// Generate dynamic metadata based on Flagstaff contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getFlagstaffContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Flagstaff Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Flagstaff, AZ'
    
    return {
      title: `Flagstaff Insulation Contractors, Find Top Rated Local Companies Near You`,
      ...
      keywords: [
        ...
        'Flagstaff attic insulation',
        'Flagstaff spray foam insulation',
        'Flagstaff wall insulation',
        'Flagstaff basement insulation',
        'Flagstaff crawl space insulation',
        'Flagstaff energy efficiency',
        'Arizona insulation services',
        'Flagstaff insulation quotes',
        'licensed insulation contractors Flagstaff',
        'verified insulation professionals Flagstaff'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/flagstaff-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Flagstaff Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Flagstaff, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Flagstaff contractors from Supabase database
async function getFlagstaffContractors() {
  try {
    // Get contractors based in Flagstaff (business city from their profile)
    ...
    // Filter contractors who serve Flagstaff:
    // 1. Contractors based in Flagstaff (business_city matches)
    // 2. Contractors who have Flagstaff in their service areas
    const targetCity = 'flagstaff'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const flagstaffContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Flagstaff
      const isBasedInFlagstaff = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Flagstaff via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesFlagstaff = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInFlagstaff || servesFlagstaff
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = flagstaffContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Flagstaff
        const isBasedInFlagstaff = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInFlagstaff // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch Flagstaff contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Flagstaff
    ...
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Flagstaff%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Flagstaff reviews found, fallback to Phoenix reviews
...
// Add Flagstaff area cities with 50k+ population
const flagstaffAreaCities = [
...
export default async function FlagstaffInsulationContractors() {
  ...
  const cityStats = {
    totalReviews: flagstaffContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: flagstaffContractors.length > 0 ? 
      (flagstaffContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / flagstaffContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: flagstaffContractors.length,
    jobsCompleted: flagstaffContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-700 mb-8">
              ...
              licensed professionals who understand Flagstaff's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Flagstaff</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4">
          ...
          <div className="space-y-8">
            {flagstaffContractors.map((contractor: any) => (
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {flagstaffAreaCities.map((city, index) => (
              city.name === 'Flagstaff' ? (
        ...
        <div className="container mx-auto px-4">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Flagstaff, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Flagstaff and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Flagstaff,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Flagstaff Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Flagstaff insulation contractors
            </p>
          </div>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Comprehensive insulation solutions for every part of your Flagstaff home
            </p>
          </div>
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Flagstaff Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Flagstaff's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Flagstaff Quotes Now
          </QuoteButton>
        </div>
      </section>
