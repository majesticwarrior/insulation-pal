  ...
  try {
    ...
    if (error) {
      ...
      if (error.message && error.message.includes('bbb_accredited')) {
        ...
        // Filter contractors who serve Phoenix:
        // 1. Contractors based in Phoenix (business_city matches)
        // 2. Contractors who have Phoenix in their service areas
        const targetCity = 'phoenix'
        const targetCityNormalized = targetCity.toLowerCase().trim()
        
        const phoenixContractorsRetry = contractorsRetry?.filter((contractor: any) => {
          // Check if contractor is based in Phoenix
          const isBasedInPhoenix = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
          
          // Check if contractor serves Phoenix via service areas
          ...
          return isBasedInPhoenix || servesPhoenix
        }) || []
        ...
        const uniqueContractors = phoenixContractorsRetry.reduce((acc: any[], contractor: any) => {
          ...
          if (!existingContractor) {
            ...
            acc.push({
              ...
              isBasedInCity: isBasedInPhoenix // Flag to sort city-based contractors first
...
export default async function PhoenixInsulationContractors() {
  ...
  const serviceDescriptions = getCityServiceDescriptions('Phoenix')
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 mb-8">
              Comprehensive insulation solutions for every part of your Phoenix home
            </p>
          </div>
