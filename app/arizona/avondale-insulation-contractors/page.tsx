...
// Generate dynamic metadata based on Avondale contractor data
export async function generateMetadata(): Promise<Metadata> {
  try {
    const contractors = await getAvondaleContractors()
    const contractorCount = contractors.length
    const topContractor = contractors[0]
    
    const baseTitle = 'Avondale Insulation Contractors'
    const baseDescription = 'Find the best insulation contractors in Avondale, AZ'
    
    return {
      title: 'Insulation Contractors in Avondale, AZ, Expert Licensed Companies - InsulationPal',
      ...
      keywords: [
        'Avondale insulation contractors',
        'Avondale attic insulation',
        'Avondale spray foam insulation',
        'Avondale wall insulation',
        'Avondale basement insulation',
        'Avondale crawl space insulation',
        'Avondale energy efficiency',
        'Arizona insulation services',
        'Avondale insulation quotes',
        'licensed insulation contractors Avondale',
        'verified insulation professionals Avondale'
      ],
      openGraph: {
      ...
      alternates: {
        canonical: 'https://insulationpal.com/arizona/avondale-insulation-contractors',
      },
    }
  } catch (error) {
    return {
      title: 'Avondale Insulation Contractors - InsulationPal',
      description: 'Find the best insulation contractors in Avondale, AZ. Get free quotes from licensed professionals.',
    }
  }
}


// Fetch Avondale contractors from Supabase database
async function getAvondaleContractors() {
  try {
    // Get contractors based in Avondale (business city from their profile)
    ...
    // Filter contractors who serve Avondale:
    // 1. Contractors based in Avondale (business_city matches)
    // 2. Contractors who have Avondale in their service areas
    const targetCity = 'avondale'
    const targetCityNormalized = targetCity.toLowerCase().trim()
    
    const avondaleContractors = contractors?.filter((contractor: any) => {
      // Check if contractor is based in Avondale
      const isBasedInAvondale = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
      
      // Check if contractor serves Avondale via service areas
      const serviceAreas = contractor.contractor_service_areas || []
      const servesAvondale = serviceAreas.some((area: any) => {
        const areaCity = area.city?.toLowerCase().trim()
        return areaCity === targetCityNormalized
      })
      
      return isBasedInAvondale || servesAvondale
    }) || []

    // Remove duplicates and transform data to match component format
    // Also mark which contractors are based in the city vs serve via service areas
    const uniqueContractors = avondaleContractors.reduce((acc: any[], contractor: any) => {
      const existingContractor = acc.find(c => c.id === contractor.id)
      if (!existingContractor) {
        // Check if contractor is based in Avondale
        const isBasedInAvondale = contractor.business_city?.toLowerCase().trim() === targetCityNormalized
        ...
        acc.push({
          ...
          yearEstablished: contractor.founded_year || 2020,
          about: contractor.bio || 'Professional insulation contractor serving the Phoenix area.',
          isBasedInCity: isBasedInAvondale // Flag to sort city-based contractors first
          // Removed recentProjects as requested
        })
      }
      return acc
...
// Fetch Avondale contractor reviews
async function getPhoenixReviews() {
  try {
    // Try to fetch reviews with location containing Avondale
    ...
    const { data: contractorReviews, error: contractorError } = await (supabase as any)
      ...
      `)
      .eq('contractors.status', 'approved')
      .ilike('contractors.business_city', '%Avondale%')
      .order('created_at', { ascending: false })
      .limit(10)
    ...
    // If no Avondale reviews found, fallback to Phoenix reviews
...
// Add Avondale area cities with 50k+ population
const avondaleAreaCities = [
...
export default async function AvondaleInsulationContractors() {
  ...
  const cityStats = {
    totalReviews: avondaleContractors.reduce((sum: number, contractor: any) => sum + contractor.reviewCount, 0),
    averageRating: avondaleContractors.length > 0 ? 
      (avondaleContractors.reduce((sum: number, contractor: any) => sum + contractor.rating, 0) / avondaleContractors.length).toFixed(1) : 
      '4.8',
    totalContractors: avondaleContractors.length,
    jobsCompleted: avondaleContractors.reduce((sum: number, contractor: any) => sum + contractor.jobsCompleted, 0)
  }
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-bold text-[#0a4768] mb-6">
              Hire a Licensed Insulation Contractor in Avondale
            </h1>
            <p className="text-xl text-gray-700 mb-8">
              Find the best insulation contractors in Avondale, Arizona. Get free quotes from local, 
              licensed professionals who understand Avondale's unique climate challenges.
            </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="grid lg:grid-cols-3 gap-8">
            <Card>
              <CardHeader>
                <CardTitle className="text-[#0a4768]">Insulation Types for Avondale</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <section className="py-16 bg-[#D6D6D6]">
        <div className="container mx-auto px-4 max-w-[1400px]">
          ...
          <div className="space-y-8">
            {avondaleContractors.map((contractor: any) => (
      ...
      {/* Finding Reliable Insulation Contractors in Avondale */}
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="space-y-6">
            <p className="text-gray-700 leading-relaxed">
              Finding reliable insulation contractors in Avondale isn't always easy—especially if you want quality work that doesn't break the bank. We get it. Insulation projects need skilled folks who are licensed, insured, and actually care about doing things right.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              InsulationPal now connects Avondale homeowners with trusted local insulation contractors through our easy-to-use platform, making it simple to compare quotes and hire qualified professionals for any insulation project. No more guessing—our service gives you access to vetted contractors who meet real industry standards and have legit credentials.
            </p>
            
            <p className="text-gray-700 leading-relaxed">
              We built this platform because, honestly, searching for professional insulation services can be a headache. With InsulationPal, people in Avondale can grab multiple estimates, check out contractor backgrounds, and figure out what works for their home—without calling a dozen companies or stressing about scams.
            </p>
            
            <Card className="bg-[#D8E1FF] border-[#0a4768] shadow-lg mt-8">
              <CardContent className="p-6 text-center">
                <CheckCircle className="w-12 h-12 text-[#0a4768] mx-auto mb-4" />
                <h3 className="text-2xl font-bold text-[#0a4768] mb-3">Ready to Find Your Avondale Contractor?</h3>
                <p className="text-lg text-gray-700 mb-2">
                  Connect with pre-screened, licensed, and insured insulation professionals in Avondale, AZ.
                </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4 max-w-[1400px]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {avondaleAreaCities.map((city, index) => (
              city.name === 'Avondale' ? (
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="rounded-lg overflow-hidden shadow-lg">
            <iframe
              ...
              title="Avondale, Arizona - Service Area Map"
            />
          </div>
          <div className="text-center mt-6">
            <p className="text-gray-600 mb-4">
              Our certified insulation contractors serve all cities and communities throughout Avondale and the surrounding East Valley area.
            </p>
            <Button 
              asChild 
              variant="outline"
            ...
            >
              <Link 
                href="https://www.google.com/maps/place/Avondale,+AZ"
                target="_blank"
        ...
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
              Latest Avondale Reviews
            </h2>
            <p className="text-lg text-gray-600">
              See what customers are saying about our Avondale insulation contractors
            </p>
          </div>
            ...
            <div className="max-w-[1400px] mx-auto">
              ...
              <div className="text-center mt-8">
                <p className="text-gray-600 text-sm">
                  Showing {phoenixReviews.length} of the latest reviews from Avondale customers
                </p>
              </div>
            </div>
          ) : (
      ...
      {/* Recent Avondale Completed Projects */}
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-[1400px]">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0a4768] mb-4">
      ...
      <section className="py-16 bg-[#D6D6D6] text-[#0a4768]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Improve Your Avondale Home?
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto">
            Connect with Avondale's most trusted insulation contractors. Get free quotes from verified professionals 
            who understand the unique insulation needs of desert homes.
          </p>
          <QuoteButton className="bg-[#F5DD22] hover:bg-[#f0d000] text-[#0a4768] font-semibold px-8 py-3 text-lg">
            Get Avondale Quotes Now
          </QuoteButton>
        </div>
      </section>
